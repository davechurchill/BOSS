...
AStarSearchExperiment::AStarSearchExperiment()
...
AStarSearchExperiment::AStarSearchExperiment(const std::string & name, const json & val)
    : m_name(name)
    , m_searchTimeLimitMS(30000)
    , m_printNewBest(false)
    , m_smartSearch(true)
    ...
    , m_supplyBoundingThreshold(1.5)
{
    BOSS_ASSERT(val.count("State") && val["State"].is_string(), "AStarSearchExperiment must have a 'State' string");
    m_initialState = BOSSConfig::Instance().GetState(val["State"]);

    BOSS_ASSERT(val.count("Goal") && val["Goal"].is_string(), "AStarSearchExperiment must have a 'Goal' string");
    m_goal = BOSSConfig::Instance().GetBuildOrderSearchGoalMap(val["Goal"]);

    BOSS_ASSERT(val.count("SearchTimeLimitMS") && val["SearchTimeLimitMS"].is_number_integer(), "AStarSearchExperiment must have a 'SearchTimeLimitMS' int");
    ...
    if (!m_smartSearch)
    {
        BOSS_ASSERT(val.count("RelevantActions") && val["RelevantActions"].is_array(), "AStarSearchExperiment with SmartSearch=false must have a 'RelevantActions' array");
...
void AStarSearchExperiment::printResults(const DFBB_BuildOrderSearchResults & results) const
{
...
void AStarSearchExperiment::run()
{
    static std::string stars = "************************************************";
    const std::string searchLabel = m_smartSearch ? "A* Smart" : "A*";
    ...
    for (const ActionOrderingType ordering : m_orderings)
    {
        ...
        search.setSmartSearch(m_smartSearch);
        search.setOrdering(ordering);
        search.setRelevantActions(m_relevantActions);
...
void AStarSearchExperiment::writeHTMLFile(const std::vector<RunData> & runs) const
{
